...
const ProductSell = ({ cookies }) => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.NEXT_PUBLIC_PARTNER_API}/items/${router.query.id}`, {})
      .then(res => setProduct(res.data.data.item));
  }, []);
  ...
  useEffect(() => {
    onChangeValue({
      ...formValues,
      product_name: get(pdt, 'product_name', ''),
      description: get(pdt, 'description', ''),
      // category: get(pdt, 'category', ''),
    });
  }, [pdt]);
  ...
  const handleSubmit = e => {
    ...
    axios({
      method: 'PUT',
      url: `${process.env.NEXT_PUBLIC_PARTNER_API}/items/${router.query.id}`,
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${isAuthoriszed}`,
      },
      data: {
        update_status: parseInt(formValues.update_status !== '' ? formValues.update_status : 0),
  ...
  return (
    <Panel>
      <PanelHeading>
        <HeaderLable>Edit Product</HeaderLable>
      </PanelHeading>
      <PanelContentWrapper>
        <TextInputsSection>
          <InputWrapper>
