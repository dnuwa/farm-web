...
const BestSelling = ({ cookies, best_selling }) => {
  let loggedIn = cookies.get('auth_token');

  //add quantity to each item
  const modified = get(best_selling, 'data.items', []).map(obj => ({ ...obj, quantity: 1 }));
  ...
  return (
    <>
      <SectionHeader>
        <SectionHeading>BEST SELLING</SectionHeading>
...
export default withCookies(BestSelling);
