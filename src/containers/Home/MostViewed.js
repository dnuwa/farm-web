...
const MostViewed = ({ cookies, most_viewed }) => {
  let loggedIn = cookies.get('auth_token');
//add quantity to each item
const modified = get(most_viewed, 'data.items', []).map(obj => ({ ...obj, quantity: 1 }));
...
return (
  <>
    <SectionHeader>
      <SectionHeading>Most Viewed</SectionHeading>
      <Link href={`/products/most-viewed`}>
        <SectionHeading>
...
export default withCookies(MostViewed);
