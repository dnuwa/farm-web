...
const BestRated = ({ cookies, best_rated }) => {
  let loggedIn = cookies.get('auth_token');

  //add quantity to each item
  const modified = get(best_rated, 'data.items', []).map(obj => ({ ...obj, quantity: 1 }));
  ...
  return (
    <>
      <SectionHeader>
        <SectionHeading>BEST RATED</SectionHeading>
        <Link href={`/products/best-rated`}>
          <SectionHeading>
...
export default withCookies(BestRated);
